  ...
  tabItems(
    tabItem(
      ...
      div(
        ...
        h4("Welcome to PT SIPL Training Dashboard", style = "color: #555; font-size: 35px")
      ),
            ...
            fluidRow(
              ...
              column(
                ...
                div(
                  ...
                  DT::dataTableOutput("course_participants")             
                )
              )
            )
    ),
...
server <- function(input, output, session) {
  ...
  certification_data <- reactive({
    ...
    data <- data %>% mutate(exp_date = ymd(exp_date))
  ...
  count_expired <- function(status) {
    ...
    if (status == "expired_this_year") {
      ...
      return(sum(cert_data$exp_date > today & cert_data$exp_date <= today %m+% months(3)))
  ...
  observeEvent(input$btn_expired_3_months, {
    selected_data <- certification_data() %>%
      filter(exp_date > Sys.Date() & exp_date <= Sys.Date() %m+% months(3)) %>%
      mutate(No = row_number()) %>%
      select(No, `Employee Name` = name, `Department` = depart, 
             `Work Location` = work_loc, `Course Title` = title, 
             `Type` = type2, `Expired Date` = exp_date)
  ...
  output$course_stats <- renderUI({
    ...
    # Mengelompokkan berdasarkan kombinasi unik title, start_date, dan end_date
    summary_data <- title_data %>%
      group_by(title, start_date, end_date) %>%
      summarise(
        total_days = sum(m_days, na.rm = TRUE),
        total_hours = sum(m_hours, na.rm = TRUE),
        total_learn_hours = sum(learn_hours, na.rm = TRUE),
        .groups = "drop"
      )
    
    total_events <- nrow(summary_data)
    
    total_days <- sum(summary_data$total_days, na.rm = TRUE)
    total_hours <- sum(summary_data$total_hours, na.rm = TRUE)
    total_learn_hours <- sum(summary_data$total_learn_hours, na.rm = TRUE)
  ...
  output$course_participants <-  DT::renderDataTable({
    title_data <- selected_title()
    req(nrow(title_data) > 0)
    
    filtered_data <- title_data %>%
      arrange(name, start_date) %>%
      select(name, depart, emp_status, work_loc, start_date, end_date, venue) %>%
      rename(
        "Name" = name,
        "Department" = depart,
  ...
  export_title <- reactive({
    ...
    df <- title_data %>%
      ...
      arrange(name) %>%
      mutate(No = row_number()) %>%
      select(No, name, depart, emp_status, work_loc, start_date, end_date, venue) %>%
      rename(
        "No" = No,
        "Name" = name,
        "Department" = depart,
  ...
  generate_title_report <- function(course_info, df) {
    ...
    doc <- doc %>%
      body_add_flextable(flextable_df) %>%
      body_add_par(" ") %>%
      body_end_block_section(landscape)
  ...
  observeEvent(input$add_data, {
    ...
    if (any(missing_values)) {
      showNotification("Error: Semua kolom wajib diisi, kecuali Expired Date.", type = "error")
      return()
    }
  ...
  output$corr_result <- renderUI({
    ...
    if (correlation_result$p.value < 0.05) {
      ...
      if (correlation_result$estimate < 0) {
        result_text <- paste("Secara statistik, terdapat pengaruh antara pilihan tingkat kesulitan materi dengan nilai post-test yang didapatkan. Korelasi negatif (", 
        ...
        result_text <- paste("Secara statistik, terdapat pengaruh antara pilihan tingkat kesulitan materi dengan nilai post-test yang didapatkan. Korelasi positif (", 
                             round(correlation_result$estimate, 2), 
                             ") menunjukkan bahwa peserta yang menganggap materi lebih sulit cenderung memiliki nilai post-test yang lebih tinggi.")
      }
    } else {
      result_text <- "Tidak terdapat pengaruh yang signifikan antara pilihan tingkat kesulitan materi dengan nilai post-test yang didapatkan."
    }
